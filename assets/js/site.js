...
(function () {
    ...
    Array.prototype.forEach.call(document.querySelectorAll('.jm-copiar-clabe'), function (b) {
      b.addEventListener('click', function () {
        ...
        if (navigator.clipboard && navigator.clipboard.writeText) {
          navigator.clipboard.writeText(n).then(listo, respaldo);
        } else { respaldo(); }
        // sin https o sin permiso, el camino de siempre
        function respaldo() {
          var t = document.createElement('textarea');
          t.value = n; t.setAttribute('readonly', '');
  ...
  /* ---------- agregar al calendario ----------
     El archivo es estático (assets/boda.ics). Armarlo en el navegador y
     entregarlo como descarga funcionaba, pero Safari en iPhone le sacaba
     a la persona un permiso de descarga que asusta; un <a> a un archivo
     servido como text/calendar lo abre directo en Calendario.

     Lo único que decide el JS es cuál de los dos: a la ceremonia civil no
     va todo el mundo, y quien sí va la quiere en su nota del calendario. */
  function setupCalendario() {
    var a = $('jm-cal');
    if (!a) return;
    var civil = $('jm-itin-civil');
    var conCivil = !!civil && civil.style.display !== 'none';
    a.setAttribute('href', conCivil ? 'assets/boda-civil.ics' : 'assets/boda.ics');
  }
